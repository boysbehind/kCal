...
void MainWindow::get_fun(QString str)
{
    ...
    if (str=="root2"){
        ...
        if (text1.toDouble()<=0) text2=error[8];
            ...
            text2=QString::number(Exp(text2.toDouble(), 1/2));
    ...
    if (str=="root3"){
        emit send_fun("equal");
        text2=QString::number(Exp(text2.toDouble(), 1/3));
...
int word_analysis(QVector<QPair<QString, int>>& word, const QString expr)
{
    ...
    for(int i=0; i<expr.length(); ++i)
    {
        if(expr[i].unicode() =='M'){
            ...
            if (i<expr.length()-1){
                if((expr[i+1]>='0' && expr[i+1]<='9')||expr[i+1]=='M'){
        ...
        if (tmp==6){
            if (i>0){
                if (word[i-1].second==5){
                    word.insert(i-1, qMakePair('*', 3));   // (前加缺失的*
                    i++;
                }
            }
        }
